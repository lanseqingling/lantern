...
export async function clearComicData(prisma: PrismaClient, comicId: string) {
  ...
  await prisma.$transaction([
    ...
    prisma.artworkAnnotationWork.deleteMany({ where: { annotation: { projectId: { in: projectIds } } } }),
    prisma.artworkAnnotationMessage.deleteMany({ where: { annotation: { projectId: { in: projectIds } } } }),
    prisma.artworkAnnotationAttachment.deleteMany({ where: { annotation: { projectId: { in: projectIds } } } }),
    prisma.artworkAnnotationReference.deleteMany({ where: { annotation: { projectId: { in: projectIds } } } }),
    prisma.artworkAnnotation.deleteMany({ where: { projectId: { in: projectIds } } }),
