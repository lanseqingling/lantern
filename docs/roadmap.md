...
| 增强 | 对白与气泡 | 使用喊叫、低声和电子声等表现型气泡 | 通用 | `Dialogue / BalloonElement` 保留语义与可编辑参数，LCD 引用固定版本的 SVG 外观模板；三端按同一规则渲染 | 部分接入：基础形状可用 | 未登记 |
...
| 增强 | 画内效果 | 添加具有漫画感的拟声字 | 通用 | `TextElement` 保留文字语义；普通样式直接渲染，复杂变形引用 SVG 或透明 `AssetVersion`，改字时重新生成外观资源 | 未接入 | 未登记 |
