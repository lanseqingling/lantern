...
表格只展示当前入口覆盖情况，具体拆分、范围和三端差异见[漫画能力矩阵](./docs/capabilities.md)。
...
从 GitHub Release 源码包解压运行的 Lantern 会在启动后后台检查最新正式版本，同一检查结果保留 30 分钟。设置页可以手动检查并一键更新：Lantern 在停服前下载 Release ZIP、校验 `SHA256SUMS` 并完成解压，再替换服务文件并重新启动；界面显示下载与安装进度，刷新设置页后会恢复当前进度。作品数据目录不参与替换。Git 开发目录和缺少发行标记的手工源码目录不会自动覆盖，只打开对应 Release。
