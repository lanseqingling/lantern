...
export const workspaceCommandSchema = z.discriminatedUnion("type", [
  ...
  z.strictObject({ type: z.literal("move_presentation_unit"), unitId: z.string().min(1), direction: z.enum(["up", "down"]) }),
  z.strictObject({ type: z.literal("set_presentation_unit_name"), unitId: z.string().min(1), name: z.string().min(1).max(80).nullable() }),
  z.strictObject({ type: z.literal("set_presentation_unit_background"), unitId: z.string().min(1), color: z.enum(["#ffffff", "#000000"]) }),
  ...
  z.strictObject({ type: z.literal("reorder_frame_reading"), unitId: z.string().min(1), frameId: z.string().min(1), readingIndex: z.number().int().nonnegative() }),
  ...
  z.strictObject({ type: z.literal("add_overlay_layer"), unitId: z.string().min(1), layer: overlayLayerSchema }),
  z.strictObject({ type: z.literal("remove_overlay_layer"), unitId: z.string().min(1), layerId: z.string().min(1) }),
  z.strictObject({ type: z.literal("add_overlay_element"), unitId: z.string().min(1), layerId: z.string().min(1), element: frameElementSchema }),
  z.strictObject({ type: z.literal("remove_overlay_element"), unitId: z.string().min(1), layerId: z.string().min(1), elementId: z.string().min(1) }),
  z.strictObject({ type: z.literal("reorder_overlay_element"), unitId: z.string().min(1), layerId: z.string().min(1), elementId: z.string().min(1), index: z.number().int().nonnegative() }),
  z.strictObject({ type: z.literal("reorder_overlay_layer"), unitId: z.string().min(1), layerId: z.string().min(1), zIndex: z.number().int() }),
  ...
  z.strictObject({ type: z.literal("update_balloon"), unitId: z.string().min(1), frameId: z.string().min(1).optional(), layerId: z.string().min(1), elementId: z.string().min(1), changes: balloonChangesSchema }),
