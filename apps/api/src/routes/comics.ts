...
import { AssetKind, AssetLibraryStatus, ComicFormat, TaskStatus, type Prisma } from "@prisma/client";
import { z } from "zod";
...
import { createSignedAssetPath } from "../../../../packages/server/src/signed-assets";
...
}).refine((value) => value.title !== undefined || value.summary !== undefined || value.worldSummary !== undefined || value.canvasPageMode !== undefined);
...
export function registerComicRoutes(app: FastifyInstance) {
  ...
  app.get<{ Params: { comicId: string } }>("/v1/comics/:comicId/assets", async (request) => {
    const user = await currentUser(request);
    const comic = await prisma.comic.findFirst({ where: { id: request.params.comicId, ownerUserId: user.id, archivedAt: null }, select: { id: true } });
    if (!comic) throw new AppError("not_found", "漫画不存在。", 404);
    const assets = await prisma.asset.findMany({
      where: { ownerUserId: user.id, archivedAt: null, kind: { not: AssetKind.GENERATED_IMAGE }, libraryStatus: AssetLibraryStatus.LIBRARY, project: { chapter: { comicId: comic.id, archivedAt: null } } },
      include: { project: { select: { chapterId: true } }, versions: { where: { objectKey: { not: null } }, orderBy: { version: "desc" }, take: 1 } },
      orderBy: [{ updatedAt: "desc" }, { createdAt: "desc" }],
    });
    return ok(request, assets.map((asset) => {
      const version = asset.versions[0];
      return { id: asset.id, chapterId: asset.project.chapterId, kind: asset.kind.toLowerCase(), name: asset.name, description: asset.description, attributes: asset.attributes, versionId: version?.id, contentUrl: version ? createSignedAssetPath(version.id) : undefined, updatedAt: asset.updatedAt.toISOString() };
    }));
  });
  ...
  app.post("/v1/comics", async (request) => {
    ...
    const comic = await prisma.comic.create({ data: { ownerUserId: user.id, title: body.title, summary: body.summary, worldSummary: body.worldSummary ?? "", format, canvasPageMode: body.canvasPageMode === "spread" ? "SPREAD" : "SINGLE" } });
    return ok(request, { comic: { id: comic.id, title: comic.title } });
  });

  app.patch<{ Params: { comicId: string }; Body: { title?: string; summary?: string } }>("/v1/comics/:comicId", async (request) => {
    const user = await currentUser(request);
    ...
    if (!comic) throw new AppError("not_found", "漫画不存在。", 404);
    return ok(request, await prisma.comic.update({ where: { id: comic.id }, data: { ...(body.title !== undefined ? { title: body.title } : {}), ...(body.summary !== undefined ? { summary: body.summary } : {}), ...(body.worldSummary !== undefined ? { worldSummary: body.worldSummary } : {}), ...(body.canvasPageMode !== undefined ? { canvasPageMode: body.canvasPageMode === "spread" ? "SPREAD" : "SINGLE" } : {}) } }));
  });
