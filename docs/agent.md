...
其中，外置 Agent 已可管理页漫页面与真正双页，创建、复制和调整画格，把固定图片放入画格或纸面，并完成对白、气泡、旁白、裁切、层级、出血、frame-anchored 破格和中缝安全的单气泡跨页；任意跨展示单元、多对象重排、自动避让与高级精修仍未闭环。漫画解析目前只能依靠 Skill 形成理解或草案，尚未具备跨页解析与稳定模板对象；外置协作主要依赖宿主 Agent 与知识型或只读检查 Skill，并非 Lantern 内的 Workflow 产品能力。
