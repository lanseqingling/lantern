...
export async function seedCampusLetter() {
  ...
  await prisma.chapter.create({ data: { id: ids.chapter, ownerUserId: owner.id, comicId: ids.comic, number: 1, title: "旧看台的来信", summary: "夏葵认出书包里那枚熟悉的五瓣花印。她循着信上的约定奔向旧看台，在看见林澄的瞬间终于喊出她的名字。", coverObjectKey: cover.objectKey, coverContentType: cover.contentType, coverWidth: cover.width, coverHeight: cover.height } });
  ...
  const contextSnapshot = { comic: { id: ids.comic, title: "风停之前" }, chapter: { id: ids.chapter, title: "旧看台的来信" }, workingRevision: 1, storyboardBeats: [], assets: [], recentConversation: [] };
  const tasks = [
