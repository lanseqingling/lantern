...
| 6 | 外置 Agent、MCP 与 Skill | 已落地 | 外置 Agent 已可发现受控上下文及作品、章节、结构化资产管理能力，并继续复用同一套领域服务、ChangeSet、Task 和 Candidate | 接入方式不改变权限、目标范围、确认和写入边界，也不强制所有编辑进入 Workflow |
...
| 作品组织 | 创建、编辑和删除漫画与章节 | 已接入 | 外置 Agent 已登记列表、读取、创建、更新、深度复制和归档能力；使用 Resource Reference 固定目标 |
...
| 资产管理 | 上传、摆放、整理、派生和复用人物、场景、风格与其他图片 | 已接入 | 外置 Agent 已可读取、创建、更新和归档角色、场景、道具及参考资料卡；图片版本、派生形态与画布操作尚未开放 |
