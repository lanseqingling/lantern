...
| 6 | 外置 Agent、MCP 与 Skill | 已落地 | 外置 Agent 已可发现受控上下文及作品、章节、结构化资产、固定图片版本和派生形态管理能力，并继续复用同一套领域服务、ChangeSet、Task 和 Candidate | 接入方式不改变权限、目标范围、确认和写入边界，也不强制所有编辑进入 Workflow；同步写入要求幂等键并记录审计 |
...
| 11 | 复杂精修 | 规划 | 支持选区、遮罩、扩图、局部重画、受保护区域和候选派生 | 固定目标、参考版本与保护范围；每次结果仍是 Candidate |
...
本表说明编辑器能力、产品内 Agent 权限与 MCP 开放范围的关系。编辑器已支持不等于 Agent 可以调用，Agent 已登记也不等于 MCP 已提供执行工具；`—` 表示尚未登记为 Agent 能力，不评价未来是否接入。MCP 列只记录外置 Agent 当前可实际调用的能力，`部分接入` 后同时写明尚未开放的边界。

| 范围 | 能力组 | 编辑器状态 | Agent 状态与边界 | MCP 接入情况 |
|---|---|---|---|---|
| Agent 基础 | 普通问答、创作讨论与图片内容理解 | 已接入 | 已接入；图片先形成只读 Observation | 部分接入；可读取受限上下文和固定图片 Observation，问答由外置 Agent 自身完成 |
| 分镜 | 为唯一目标画格创建或编辑分镜条目 | 已接入 | 已接入；只修改一个 `StoryboardBeat` 的标题与描述 | 未接入执行 |
| 格内图片 | 为唯一目标画格生成或替换主图 | 已接入 | 已接入；生成新资源版本并形成单个 Candidate | 未接入执行 |
| 资产 | 生成角色或场景资产图 | 已接入 | 已接入；确认前为暂存 Candidate，不生成风格或道具资产 | 未接入执行 |
| 候选 | 查看、预览、应用、丢弃与 revision 冲突保护 | 已接入 | 已接入；应用仍经过 owner、目标和 base revision 校验 | 部分接入；受限上下文可返回候选摘要，查询、应用和丢弃尚未开放 |
| 作品组织 | 创建、编辑和删除漫画与章节 | 已接入 | 已登记语义能力；使用 Resource Reference 固定目标 | 已接入；支持漫画与一话的列表、读取、创建、更新、深度复制和归档 |
| 页面与滚动段 | 新增、命名、复制、排序、合并、拆分和删除展示单元 | 已接入 | — | 未接入 |
| 画格编排 | 新增、复制、删除、移动、缩放、边框、四角斜切、出血与层级 | 已接入 | — | 未接入 |
| 页面骨架 | 使用常用布局预设形成页面骨架 | 未接入 | — | 未接入 |
| 图片编辑 | 上传或复用图片、放入画格、调整取景、破格、跨页或跨段 | 已接入 | — | 部分接入；可向资产上传并固定图片版本，画布放置、取景和破格尚未开放 |
| 对白与气泡 | 创建、编辑、删除、移动、缩放、旋转、尾巴、排版和基础样式 | 已接入 | — | 未接入 |
| 纸面内容 | 无格图、纸面对白、旁白、装饰和跨格合成 | 已接入 | — | 未接入 |
| 资产管理 | 上传、摆放、整理、派生和复用人物、场景、风格与其他图片 | 已接入 | 已登记角色、场景、道具和参考资料卡的结构化管理能力；画布摆放留在 LCD 编辑能力 | 已接入；支持资产卡、短时效图片上传、固定 AssetVersion、图片槽、主图和派生形态管理 |
| 阅读与输出 | 保存、撤销、重做、恢复快照、阅读预览和导出 | 已接入 | 只参与 Candidate 应用边界，不直接控制保存或导出 | 未接入 |
| 阅读节奏 | 调整画格、对白阅读顺序、留白、重复和场景停顿 | 未接入 | — | 未接入 |
| 画内效果 | 速度线、集中线、冲击、光影、天气和复杂拟声字 | 部分接入 | — | 未接入 |
| 表现型气泡 | 喊叫、低声、电子声等可版本化视觉外观 | 部分接入 | — | 未接入 |
| 页面布局生成 | 根据分镜生成结构化布局 Candidate | 未接入 | — | 未接入 |
| 整话与多对象生成 | 整话分镜、多格协同、多方案和结构重编排 | 未接入 | — | 未接入 |
| 复杂精修 | 选区、遮罩、局部重画、扩图和候选派生 | 未接入 | — | 未接入 |
| 对白 AI | 润色、压缩、改写或批量调整对白 | 未接入 | — | 未接入 |
| 专业编排 | 画格拆分合并、复杂异形格、自由嵌套和完整图层面板 | 未接入 | — | 未接入 |
