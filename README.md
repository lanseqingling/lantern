...
| 页漫正文、封面、过场与真正双页管理 | ✅ | ✅ | ❌ |
| 条漫滚动段与跨段编排 | ✅ | ❌ | ❌ |
| 画格创建、边框、层级、出血、阅读顺序与跨页格 | ✅ | ✅ | ❌ |
| 外部图片上传与固定版本登记 | ✅ | ✅ | ❌ |
| 格内、纸面与跨页图片放置、裁切与有限破格 | ✅ | ✅ | 🟡 |
| 同源透明前景绑定真出格 | 🟡 | ✅ | ❌ |
| 单格分镜条目创建与编辑 | ✅ | ❌ | ✅ |
| 对白、气泡、旁白与基础文字编排 | ✅ | ✅ | ❌ |
...
第一次使用可以从 [外部 Agent 创作快速上手](./docs/agent-quickstart.md) 开始，直接复制几个简单 Prompt，体验创建作品、编排页面、继续修改和视觉检查。也可以在工作台右侧批注输入区写下修改要求，通过底部工具引用一个或多个纸面坐标，或从对象工具栏引用页面、画格、图片、对白、旁白和效果；无引用文字与混合引用都可以直接添加。Agent 会通过 MCP 读取这些精确引用并把统一方案交回 Lantern。外部 Agent 完成一话内容任务后会返回本地审查链接；你也可以从工作台右上角的历史入口查看方案，决定应用并保存、保留或丢弃。
