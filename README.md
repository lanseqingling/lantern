...
| 范围 | 当前可用能力 |
|---|---|
| 作品组织 | 创建、编辑和删除漫画与章节，在工作稿中持续创作并显式保存版本 |
| 分镜与页面 | 为画格创建和编辑分镜描述；新增、命名和删除漫画页或滚动段，并调整滚动段比例 |
| 画布编辑 | 新增、复制、删除、移动和缩放画格，放入、更换、移除格内图片并调整取景，支持多选移动和原子撤销 |
| 对白与气泡 | 新增、复制、编辑和删除对白，调整气泡位置、尺寸、尾巴和普通、思考、旁白等基础形状 |
| 资产与参考 | 上传并整理人物、场景和风格图片，在画布摆放参考、显式引用或沉淀为可复用资产 |
| 阅读与输出 | 页漫单页/双页预览、条漫连续预览与设备视区辅助；保存快照并下载当前范围 PNG 或完整 LCD |
