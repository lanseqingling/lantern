...
- `apps/web/public/samples/<sample-id>/`：该样例所需的静态源图，仅在 seed 或离线 fixture 中读取。
...
新增样例时，新增目录和对应静态目录，并在 `package.json` 添加一个明确的 `db:sample:<id>` 命令；不要再把图片、seed 或 mock 数据散落到其他应用目录、`prisma/` 或通用 fixture 目录。
